...
/**
 * Dashboard panel for Patients to view personal appointments, book new slots,
 * and view issued prescriptions.
 */
public class PatientPanel extends JPanel {
	...
	private void initUI() {
		...
		String[] apptCols = { "Appt ID", "Doctor Name", "Date", "Time", "Reason for Visit", "Fee (RM)", "Status",
				"Notes" };
		...
		String[] rxCols = { "Prescription ID", "Doctor Name", "Medication Name", "Dosage", "Instructions", "Status",
				"Issued Date" };
	...
	private void handleViewRxDetails() {
		...
		if (row == -1) {
			JOptionPane.showMessageDialog(this, "Please select a prescription from the table first.",
					"Selection Required", JOptionPane.WARNING_MESSAGE);
			return;
		}
	...
	private void showFullApptDialog(Appointment appt) {
		JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this),
				"Appointment Details - " + appt.getAppointmentId(), true);
		...
		addFormRow(panel, gbc, "Date & Time:",
				new JLabel(appt.getAppointmentDate() + " at " + appt.getAppointmentTime()), y++);
		...
		JTextArea notesArea = new JTextArea(
				appt.getConsultationNotes() != null ? appt.getConsultationNotes() : "No notes recorded.");
	...
	private void showFullRxDialog(Prescription rx) {
		JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this),
				"Prescription Details - " + rx.getPrescriptionId(), true);
		...
		addFormRow(panel, gbc, "Dispensing Pharmacy:",
				new JLabel(rx.getDispensedByPharmacistName() + " (" + rx.getPharmacySection() + ")"), y++);
		...
		JTextArea instArea = new JTextArea(
				rx.getInstructions() != null ? rx.getInstructions() : "No special instructions.");
	...
	public void refreshTables() {
		...
		for (Appointment a : appts) {
			appointmentTableModel.addRow(new Object[] { a.getAppointmentId(), a.getDoctorName(),
					a.getAppointmentDate().toString(), a.getAppointmentTime().toString(), a.getReason(),
					String.format("RM %.2f", a.getFee()), a.getStatus().getLabel(),
		...
		for (Prescription rx : rxs) {
			prescriptionTableModel.addRow(
					new Object[] { rx.getPrescriptionId(), rx.getDoctorName(), rx.getMedicationName(), rx.getDosage(),
							rx.getInstructions(), rx.getStatus().getLabel(), rx.getFormattedIssuedAt() });
		}
	}
	...
	private void showBookDialog() {
		...
		if (doctors.isEmpty()) {
			JOptionPane.showMessageDialog(this, "No doctors currently available.", "Booking Info",
		...
		String[] timeSlots = { "09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "14:00", "14:30", "15:00", "15:30",
				"16:00" };
		...
		confirmBtn.addActionListener(e -> {
			try {
				...
				if (reason.isEmpty()) {
					JOptionPane.showMessageDialog(dialog, "Reason for visit cannot be empty.", "Validation Error",
							JOptionPane.WARNING_MESSAGE);
					return;
				}

				appointmentService.bookAppointment(patient.getUserId(), selectedDoc.getUserId(), d, t, reason);
				JOptionPane.showMessageDialog(dialog, "Appointment booked successfully!", "Success",
						JOptionPane.INFORMATION_MESSAGE);
				dialog.dispose();
				refreshTables();
			} catch (Exception ex) {
	...
	private void showRescheduleDialog() {
		int row = appointmentTable.getSelectedRow();
		if (row == -1) {
			JOptionPane.showMessageDialog(this, "Please select an appointment to reschedule.", "Selection Required",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		...
		JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this),
				"Reschedule Appointment - " + apptId, true);
		...
		saveBtn.addActionListener(e -> {
			try {
				...
				JOptionPane.showMessageDialog(dialog, "Appointment rescheduled successfully!", "Success",
						JOptionPane.INFORMATION_MESSAGE);
				dialog.dispose();
				refreshTables();
			} catch (Exception ex) {
	...
	private void handleCancelAppointment() {
		int row = appointmentTable.getSelectedRow();
		if (row == -1) {
			JOptionPane.showMessageDialog(this, "Please select an appointment to cancel.", "Selection Required",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		...
		String reason = JOptionPane.showInputDialog(this,
				"Please state the reason for cancelling appointment " + apptId + ":", "Cancel Appointment",
				JOptionPane.QUESTION_MESSAGE);

		if (reason != null && !reason.trim().isEmpty()) {
			try {
				appointmentService.cancelAppointment(apptId, reason.trim());
				JOptionPane.showMessageDialog(this, "Appointment cancelled.", "Cancelled",
						JOptionPane.INFORMATION_MESSAGE);
				refreshTables();
			} catch (Exception ex) {
